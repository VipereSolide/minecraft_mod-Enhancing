...
package net.mcreator.enhancing.item.inventory;
...
public class QuiverInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	...
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntity().getItem().getItem() == EnhancingModItems.QUIVER.get()) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(9) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != EnhancingModItems.QUIVER.get();
			}
