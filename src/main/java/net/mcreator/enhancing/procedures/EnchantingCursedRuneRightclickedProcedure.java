...
public class EnchantingCursedRuneRightclickedProcedure
{
	...
	public static void ApplyEnchantments(LevelAccessor _world, Entity _entity, Map<Enchantment, Integer> _enchantments, Map.Entry<Enchantment, Integer> _entry, ItemStack _mainHandItemStack, ItemStack _itemstack)
	{
		...
		if (_entry.getValue() - 1 <= 0)
		{
			_enchantments.remove(_entry.getKey());
		}
		else
		{
		...
		if (_entity instanceof LivingEntity __entity)
		{
			...
			if (_world instanceof Level __level)
			{
				...
				{
					__level.playSound(null, new BlockPos(__entity.getX(), __entity.getY(), __entity.getZ()), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("entity.vex.ambient")), SoundSource.PLAYERS, 1, 1);
					__level.playSound(null, new BlockPos(__entity.getX(), __entity.getY(), __entity.getZ()), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.enchantment_table.use")), SoundSource.PLAYERS, (float) 0.75f, 1);
				}
				else
				{
					__level.playLocalSound(__entity.getX(), __entity.getY(), __entity.getZ(), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("entity.vex.ambient")), SoundSource.PLAYERS, 1, 1, false);
					__level.playLocalSound(__entity.getX(), __entity.getY(), __entity.getZ(), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.enchantment_table.use")), SoundSource.PLAYERS, (float) 0.75f, 1, false);
				}
			}
